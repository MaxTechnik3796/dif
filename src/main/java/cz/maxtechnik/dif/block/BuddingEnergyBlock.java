...
public class BuddingEnergyBlock extends AmethystBlock{
	...
	public void randomTick(@NotNull BlockState blockState,@NotNull ServerLevel world,@NotNull BlockPos pos,RandomSource source){
		if(source.nextInt(5)==0){
			...
			Block block=null;
			if(canClusterGrowAtState(blockstate)) block=DifModBlocks.SMALL_ENERGY_BUD.get();
			else if(blockstate.is(DifModBlocks.SMALL_ENERGY_BUD.get())&&blockstate.getValue(AmethystClusterBlock.FACING).equals(direction))
				block=DifModBlocks.MEDIUM_ENERGY_BUD.get();
			else if(blockstate.is(DifModBlocks.MEDIUM_ENERGY_BUD.get())&&blockstate.getValue(AmethystClusterBlock.FACING).equals(direction))
				block=DifModBlocks.LARGE_ENERGY_BUD.get();
			else if(blockstate.is(DifModBlocks.LARGE_ENERGY_BUD.get())&&blockstate.getValue(AmethystClusterBlock.FACING).equals(direction))
