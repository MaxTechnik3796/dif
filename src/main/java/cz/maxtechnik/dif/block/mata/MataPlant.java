...
public class MataPlant extends SugarCaneBlock implements BonemealableBlock{
	...
	private static final TagKey<Block> FLOWER_PLANT_SOIL=BlockTags.create(ResourceLocation.fromNamespaceAndPath(DifMod.MODID,"flower_plant_soil"));
	public MataPlant(){
	...
	public void randomTick(@NotNull BlockState blockstate,@NotNull ServerLevel world,@NotNull BlockPos pos,@NotNull RandomSource random){
		...
		while(world.getBlockState(checkPos).is(this)){
			height++;
			if(height>=DifModCommonConfig.MATA_PLANT_MAX_HEIGHT.get()) return;
		...
		}else world.setBlock(pos,blockstate.setValue(AGE,age+1),4);
	}
	@Override
