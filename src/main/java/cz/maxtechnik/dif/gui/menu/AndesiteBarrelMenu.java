...
public class AndesiteBarrelMenu extends AbstractContainerMenu implements Supplier<Map<Integer,Slot>>{
	// Konstanty pro 4-řádkovou bednu
	private static final int ROWS=4;
	private static final int SLOTS_PER_ROW=9;
	private static final int CONTAINER_SLOTS=ROWS*SLOTS_PER_ROW; // 36
	...
	public AndesiteBarrelMenu(int id,Inventory inv,FriendlyByteBuf extraData){
		super(DifModMenus.ANDESITE_BARREL_MENU.get(),id);
		this.entity=inv.player;
		this.world=inv.player.level();
		this.internal=new ItemStackHandler(CONTAINER_SLOTS); // 36 slotů
		BlockPos pos=null;
		if(extraData!=null){
		...
		if(pos!=null){
			...
			if(boundBlockEntity!=null){
				...
				if(boundBlockEntity instanceof AndesiteBarrel be){
	...
	public @NotNull ItemStack quickMoveStack(@NotNull Player playerIn,int index){
		...
		if(slot.hasItem()){
			...
			// Pokud je to slot v barelu (index < 36)
	...
	public void removed(@NotNull Player playerIn){
		if(this.boundBlockEntity instanceof AndesiteBarrel be){
