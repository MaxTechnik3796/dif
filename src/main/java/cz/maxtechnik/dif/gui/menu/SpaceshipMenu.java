...
public class SpaceshipMenu extends AbstractContainerMenu{
		...
		if(shipInventory!=null){
			this.addSlot(new SlotItemHandler(shipInventory,0,206,114){
				...
				public boolean mayPlace(@NotNull ItemStack stack){
					return stack.is(DifModItems.ROCKET_FUEL.get()); // Povolí jen diamanty
				}
			});
			// 2. NÁKLADOVÝ PROSTOR (Indexy 1 - 8) - volitelné, pokud chceš i bednu v lodi
			for(int i=0;i<8;i++){
	...
	public @NotNull ItemStack quickMoveStack(@NotNull Player player,int index){
		...
		if(slot.hasItem()){
			...
			if(index<9){ // 1 palivo + 8 náklad = 9 slotů lodi
