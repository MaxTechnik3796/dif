...
public class ForgeRadialScreen extends Screen {
	...
	private int cols()  { return Math.clamp(tankIndices.size(), 1, COLS); }
	private int gridX() { return width  / 2 - (cols() * (CARD_W + CARD_GAP) - CARD_GAP) / 2; }
	private int gridY() {
	...
	public void render(@NotNull GuiGraphics gfx, int mx, int my, float partial) {
		...
		for (int i = 0; i < total; i++)
			drawCard(gfx, i, hoveredIdx == i, selectedIdx == i, pulse, elapsed);
	...
	private static int getFluidColor(FluidStack fs) {
		...
		} catch (Exception e) { return 0xFF6600; }
	}
