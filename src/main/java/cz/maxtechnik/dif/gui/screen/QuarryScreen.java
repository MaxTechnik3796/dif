...
public class QuarryScreen extends AbstractContainerScreen<QuarryMenu>{
	...
	protected void renderLabels(GuiGraphics g,int mouseX,int mouseY){
		...
		g.drawString(this.font,"Area: "+this.menu.getAreaX()+" x "+this.menu.getAreaZ(),tx,33,0xFFFFFF,false);

		if(state!=4){
			g.drawString(this.font,"Power: "+this.menu.getTotalQP()+" QP",tx,45,0xFFFFFF,false);
			g.drawString(this.font,"Usage: "+formatValue(this.menu.getFECost())+" FE/t",tx,57,0xFFFFFF,false);
		}
	}

	private String formatValue(int value){
		if(value>=1000000) return String.format("%.1fM",value/1000000f);
		if(value>=1000) return String.format("%.1fK",value/1000f);
