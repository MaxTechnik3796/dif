...
public class QuarryRenderer implements BlockEntityRenderer<QuarryBlockEntity>{
	...
	public void render(QuarryBlockEntity be,float partialTick,@NotNull PoseStack ps,
		...
		wirePillar(m,vc,minX,minZ,yBot,yTop,FRAME_COLOR);
		wirePillar(m,vc,maxX,minZ,yBot,yTop,FRAME_COLOR);
		wirePillar(m,vc,minX,maxZ,yBot,yTop,FRAME_COLOR);
		wirePillar(m,vc,maxX,maxZ,yBot,yTop,FRAME_COLOR);
	...
	public static void register(QuarryLandmarkBlockEntity lm){ FORMED_LANDMARKS.put(lm.getBlockPos(),lm); }
	public static void unregister(BlockPos pos){ FORMED_LANDMARKS.remove(pos); }
	...
	private static void wirePillar(Matrix4f m,VertexConsumer vc,float x,float z,float yBot,float yTop,int[] c){
		wireLine(m,vc,x,yBot,z,x,yTop,z,c);
	}
