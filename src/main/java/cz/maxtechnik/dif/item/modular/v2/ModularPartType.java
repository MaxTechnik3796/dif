...
public enum ModularPartType{
	NONE("none",new ModularParts[]{}),
	HANDLE("handle",new ModularParts[]{ModularParts.HANDLE}),
	BINDING("binding",new ModularParts[]{ModularParts.BINDING,SWORD_BINDING}),
	HEAD("head",new ModularParts[]{AXE_HEAD,PICKAXE_HEAD,SWORD_HEAD,SHOVEL_HEAD,HOE_HEAD,BATTLE_AXE_HEAD,KATANA_HEAD,TIMBER_AXE_HEAD,HAMMER_HEAD,EXCAVATOR_HEAD});
	...
	public static ModularPartType getPartType(ModularParts part){
		if(isHead(part)) return HEAD;
		if(isBinding(part)) return BINDING;
		if(isHandle(part)) return HANDLE;
		return NONE;
	}
