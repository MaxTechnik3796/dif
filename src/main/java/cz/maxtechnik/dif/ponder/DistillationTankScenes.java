...
public class DistillationTankScenes{
	public static void intro(SceneBuilder sceneBuilder,SceneBuildingUtil util){
		...
		scene.idle(30);
		narrate(scene,"The distillation tank is a multi-block structure that can be built as 1x1, 2x2, or 3x3, its height is determined by the recipe.",util.vector().centerOf(3,4,3),30);
		scene.idle(10);
		Selection inputSelection=util.select().fromTo(0,1,3,2,2,8);
		reveal(scene,inputSelection,Direction.EAST);
		narrate(scene,"The input tank is located at the very bottom.",util.vector().centerOf(3,1,4),20);
		...
		narrate(scene,"For example, Crude Oil.",util.vector().centerOf(1,2,5),5);
		...
		scene.idle(20);
		narrate(scene,"Don't forget to turn on the burners!",util.vector().centerOf(3,0,4),8);
		scene.idle(8);
		showClickWithItemAt(scene,util,new BlockPos(4,0,3),new ItemStack(Items.COAL),Pointing.RIGHT,Type.RIGHT,4);
		scene.idle(8);
		...
		scene.idle(20);
		narrate(scene,"The Distillation Tank will now begin processing fluids.",util.vector().centerOf(3,4,3),15);
