...
/**
 *Shared authoring helpers for the mod's ponder scenes: baseplate setup, timing shortcuts, and
...
public abstract class PonderScene{
	...
	/**Configure and reveal the baseplate for a scene of the given footprint.*/
	...
	public static int textDuration(String text){
		...
		return Math.clamp(30+words*6L,50,130);
	}
	...
	public static void goggleHint(SceneBuilder scene,SceneBuildingUtil util,BlockPos pos){
		scene.overlay().showControls(util.vector().topOf(pos),Pointing.DOWN,40)
				.withItem(new ItemStack(Objects.requireNonNull(BuiltInRegistries.ITEM.get(ResourceLocation.parse("create:goggles")))));
	}
	...
	/**Click prompt holding an item above a block; chain e.g. {@code .whileSneaking()} on the result.*/
	public static InputElementBuilder showClickWithItemAt(SceneBuilder scene,SceneBuildingUtil util,BlockPos pos,ItemStack itemStack,Pointing pointing,Type type,int duration){
		return showClickWithoutItem(scene,util,pos,type,pointing,duration).withItem(itemStack);
	}
	/**Click prompt holding an itemStack above a block, pointing down for the default duration.*/
	public static InputElementBuilder showClickWithItemAt(SceneBuilder scene,SceneBuildingUtil util,BlockPos pos,ItemStack itemStack,Type type){
		return showClickWithItemAt(scene,util,pos,itemStack,Pointing.DOWN,type,DEFAULT_DELAY);
	}
	/**Click prompt with empty hand above a block, pointing down for the default duration.*/
	public static InputElementBuilder showClickWithoutItem(SceneBuilder scene,SceneBuildingUtil util,BlockPos pos,Type type){
		return showClickWithoutItem(scene,util,pos,type,Pointing.DOWN,DEFAULT_DELAY);
	}
	/**Click prompt with empty hand above a block.*/
	public static InputElementBuilder showClickWithoutItem(SceneBuilder scene,SceneBuildingUtil util,BlockPos pos,Type type,Pointing pointing,int duration){
		if(type.equals(Type.RIGHT)) return scene.overlay().showControls(util.vector().topOf(pos),pointing,duration).rightClick();
		else return scene.overlay().showControls(util.vector().topOf(pos),pointing,duration).leftClick();
	}
	/**Show item above block.*/
	public static InputElementBuilder showItem(SceneBuilder scene,SceneBuildingUtil util,BlockPos pos,ItemStack itemStack,Pointing pointing,int duration){
		return scene.overlay().showControls(util.vector().topOf(pos),pointing,duration).withItem(itemStack);
	}
	/**Fill FluidTank with some fluid; needs the Create scene builder.*/
