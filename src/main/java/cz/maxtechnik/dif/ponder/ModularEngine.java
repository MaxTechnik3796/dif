...
public class ModularEngine{
	public static void intro(SceneBuilder sceneBuilder,SceneBuildingUtil util){
		...
		scene.idle(8);
		narrate(scene,"Modular Engine is a multi-block structure created from Engine Base and Engine Extender.",util.vector().centerOf(3,1,3));
		scene.idle(4);
		...
		scene.idle(8);
		narrate(scene,"There can be up to 3 Engine Extenders.",util.vector().centerOf(2,1,3));
		scene.idle(4);
		...
		scene.idle(8);
		narrate(scene,"Engine can by stopped by redstone.",util.vector().centerOf(2,1,1));
		scene.idle(4);
