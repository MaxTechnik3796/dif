...
public class PortableEngine{
	public static void intro(SceneBuilder sceneBuilder,SceneBuildingUtil util){
		...
		scene.title("portable_engine_intro","Portable Engine");
		setupScene(7,scene);
		zoom(scene,0.9F);
		scene.idle(10);
		reveal(scene,util.select().position(3,1,3),Direction.DOWN);
		scene.idle(8);
		narrate(scene,"Portable Engine is a single-block variant of a Modular Engine.",util.vector().centerOf(3,1,3));
		scene.idle(4);
		...
		scene.idle(8);
		narrate(scene,"You can use any supported fuel.",util.vector().centerOf(1,2,6));
		scene.idle(4);
		...
		narrate(scene,"The direction of rotation can be changed by clicking with the Wrench on the side with the axis.",util.vector().centerOf(3,1,3));
		applyKineticSpeedAt(scene,util,outputSelection,-192);
		scene.effects().rotationDirectionIndicator(new BlockPos(3,1,0));
		scene.idle(10);
		Selection redstoneSelection=util.select().fromTo(1,1,3,2,1,3);
		reveal(scene,redstoneSelection,Direction.EAST);
		scene.idle(8);
		narrate(scene,"Engine can by stopped by redstone.",util.vector().centerOf(1,1,3));
		scene.idle(4);
